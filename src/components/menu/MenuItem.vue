...
import ForwardButton from "@/components/direction-button/VForwardButton.vue";
...
const getPathOfMenuType = (menuType: MenuType): string => {
  switch (menuType) {
    case '서평관리':
      return '/review';
    default:
...
const moveToMenuTypePage = (menuType: MenuType) => {
  let path: string = getPathOfMenuType(menuType);
  useRouter().push(path);
}
</script>
